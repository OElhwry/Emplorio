...
/** Reads the app theme, which is driven by `data-theme` on <html> (see layout.tsx). */
...
export type SpiralTone = 'auto' | 'light' | 'dark';

export type SpiralLoaderProps = {
  size?: number;
  className?: string;
  /**
   * Stroke colour of the spiral. The raw animation is white.
   * - `auto` (default): white in dark theme, inverted to dark in light theme.
   * - `light`: always white. Use on accent / coloured backgrounds (e.g. primary buttons).
   * - `dark`: always inverted to dark. Use on light surfaces.
   */
  tone?: SpiralTone;
};

export function SpiralLoader({ size = 16, className, tone = 'auto' }: SpiralLoaderProps) {
  ...
  if (!isMounted) return <span style={{ width: size, height: size, flexShrink: 0 }} aria-hidden />;

  const needsInvert = tone === 'dark' || (tone === 'auto' && !isDark);
  ...
  return (
    ...
    >
      <span style={layerStyle(phase === 'fast')}>
        <Lottie
          lottieRef={fastRef}
      ...
      <span style={layerStyle(phase === 'slow')}>
        <Lottie
          lottieRef={slowRef}
